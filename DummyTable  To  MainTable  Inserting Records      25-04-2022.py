...
def post_into_main_table():
    record = session.query(CustomerProductEnquiry).filter(CustomerProductEnquiry.isProcessed.is_(False)).\
            filter(CustomerProductEnquiry.errorMsg == "").all()
    record_dict = [item.__dict__ for item in record]
    return record_dict


@app.route('/post_data_to_main_table', methods = ['POST'])
def post_data_to_main_table():
    """Function used to insert record into main table"""
    try:
        record_dict1 = post_into_main_table()
        
        print("record_dict ---  >", record_dict1)

        if record_dict1:
            for item in record_dict1:

                result = ProductEnquiry(customerName=item.get('customerName' , ""),
                                        ...
                                        city=item.get('city',""),
                                        existingVehicle=item.get('existingVehicle',""),
                                        wantTestDrive=item.get('wantTestDrive',False),
                                        briefAboutEnquery=item.get('briefAboutEnquery',""),
                                        expectedDateOfPurchase=item.get('expectedDateOfPurchase',""),
                                        gender=item.get('gender',""),
                                        age=item.get('age',None),
                                        occupation=item.get('occupation',""),
                                        intendedUsage=item.get('intendedUsage',""),
                                        createdDate=item.get('createdDate',""),
                                        dealerCode=item.get('dealerCode',""),
                                        dealerState=item.get('dealerState',""),
                                        dealerTown=item.get('dealerTown',""),
                                        isNew=1,
                                        dealerName=item.get('dealerName',""))

                result1 = session.query(CustomerProductEnquiry).filter(CustomerProductEnquiry.mobileNumber == item['mobileNumber']).\
                                                                        update({'isProcessed':True},synchronize_session='fetch')
